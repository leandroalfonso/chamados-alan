...
try {
    ...
    $stmt = mysqli_prepare($conn, "SELECT sr.*, c.id_usuario, c.id_chamado, c.status as status_chamado, u.nome as nome_tecnico 
    ...
    try {
        ...
        $stmt = mysqli_prepare($conn, "UPDATE solicitacoes_reabertura SET status = 'Rejeitada', data_resposta = NOW(), id_responsavel = ? WHERE id_solicitacao = ?");
        mysqli_stmt_bind_param($stmt, "ii", $id_tecnico, $id_solicitacao);
        mysqli_stmt_execute($stmt);

        // Se o chamado estiver em andamento, atualizar para fechado
        if ($solicitacao['status_chamado'] === 'Em andamento') {
            $stmt = mysqli_prepare($conn, "UPDATE chamados SET status = 'Fechado' WHERE id_chamado = ?");
        ...
        $_SESSION['success_message'] = "Solicitação de reabertura rejeitada com sucesso!";
    } catch (Exception $e) {
